...
Основные методы узлов в Godot. Они выполняются во время активности игры и в них происходит вся основная логика объектов.  Отличие в том, как выполняется _process() и _physic_process(), _process() вызывается каждый кадр и его выполнение напрямую зависит от FPS пользователя, а _physic_process() выполняются каждый фиксированный промежуток времени, каждый тик.

## Как создать и использовать таймер (Timer) в Godot?
Таймер это особый тип узла. Создается он также как и любой иной узел либо вручную либо через скрипт посредством выполнения:
```gdscript
var Timer = Timer.new()
...
Слои и маски определяют то, каким образом различные объекты будут взаимодействовать между собой при физическом взаимодействии. Параметр Layers объекта определяет то, на каком слое он находится сам, а слой Masks определяет то, с объектами на каких слоях может взаимодейтсвовать объект. Например у нас есть три слоя: Players, Enemies, Walls. узел Player имеет слой Players, узел Enemy имеет слой Enemies, при этом в масках у Players будет Enemies и Walls, чтобы корректно обрабатывать столкновения со стенами и удары врагов, а Enemy же будет иметь в масках Players и Walls.

## Как в GDScript организовать взаимодействие между разными сценами или узлами?
Взаимодействие в GDScript между разными сценами и узлами может быть осуществлено многими способами. Можно связать несколько узлов с помощью сигналов. Для обмена глобальными данными лучше всего использовать глобальные скрипты, автозагружаемые при выполнении. Например очки, в глобальном скрипте GlobalScript.gd объявляем var scores = 0 и в любом узле их можно использовать, например увелчивать: GlobalScript.scores += 1. Также взаимодействие может быть организовано и обыкновенной ссылкой на узел. var node = $Node2d как пример. Позволяет использовать все поля и методы объекта.
...
4) Профайлер. Позволяет отслеживать сколько времени и ресурсов занимает каждая функция в игре.
